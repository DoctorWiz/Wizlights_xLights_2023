/*{
  "CREDIT": "Uara Lab, forked from mojovideotech",
  "CATEGORIES": [
    "space"
  ],
  "INPUTS": [
...
void main() {
    ...
    highp int iterateint = int(iterate);
	...
	for (int r=0; r<volsteps; r++) {
		...
		for (int i=0; i<iterations; i++) {
			...
			if (i > iterateint) { a += i > 7 ? min( 12., D) : D;}
			pa=length(p2);
		}
		a*=a*a;
		float s1 = s+zoffset;
	...
	backCol2 *= cloud;
	backCol2.b *= Blue_BackColor_2*8.0;
	backCol2.r *= Red_BackColor_2;
	backCol2.b = 0.5*mix(backCol2.g, backCol2.b, 0.8);
	backCol2.g = Green_BackColor_2;
